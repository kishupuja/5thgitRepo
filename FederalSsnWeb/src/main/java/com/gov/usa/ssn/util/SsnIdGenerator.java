...
public class SsnIdGenerator implements IdentifierGenerator {
	...
	public static final String GET_SEQ_VAL = " SELECT SSN_ID_SEQ.NEXTVAL FROM DUAL";

	public SsnIdGenerator() {
		...
		try {
			...
			try {
				...
				try {
					...
					rs1 = null;
					// execute the query
					rs1 = st.executeQuery("SELECT MAX(SSNID)FROM SSN_PROVIDER");
					if (rs1.next()) {
						count = 1;
						int currentid = rs1.getInt(1) + 1;
						System.out.println(currentid);
						st.execute("create  sequence SSN_ID_SEQ start with " + currentid + " increment by 1");

					} else {
						count = 1;
						st.execute("create  sequence SSN_ID_SEQ start with 1 increment by 1;");
